...
class StorageComponentImpl$Test extends Specification with BeforeAndAfterAll {
  sequential

  val tryMongoContext = Try(new MongoContext(MongoConfig.load()))


 // val tryMongo = Try(MongoConnection("localhost", 12345))
  ...
  def drop() = for (m <- tryMongoContext) m.dbDrop()
